...
class CustomNotion(ABC):
    ...
    async def _build_properties(
        ...
        **kwargs: Any,
        ...
        if "parent_id" in kwargs:
            relation_field = RELATION_FIELD[self.database_type]
            properties[relation_field] = self.service.build_relation_property([kwargs["parent_id"]])
    ...
    async def _build_update_properties(self, **kwargs: Any) -> Dict[str, Any]:
        ...
        if "title" in kwargs:
            properties["Project name"] = self.service.build_title_property(kwargs["title"])
