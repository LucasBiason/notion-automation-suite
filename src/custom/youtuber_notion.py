...
class YoutuberNotion(CustomNotion):
    ...
    async def create_series(
        ...
        if total_episodes <= 0:
            raise ValueError("A series must contain at least one episode")
        ...
        for episode_number, payload in enumerate(episodes_payload, start=1):
            ...
            episode_title = payload.get("title") or f"Episode {episode_number:02d}"
            ...
            created = await self.create_episode(
                ...
                status=payload.get("status") or YoutuberStatus.PARA_GRAVAR.value,
    ...
    async def create_card(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            status: Notion status label (default "Não iniciado")
    ...
    async def create_episode(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            title: Episode title (e.g., "Episode 01" or custom)
        ...
        Example:
            ...
            ...     resumo_episodio="Full series about The Legend of Heroes..."
            ... )
        """
        # Validate episode 1 has synopsis
        if episode_number == 1 and not resumo_episodio:
            raise ValueError("Episode 1 must include a synopsis (resumo_episodio)")
        ...
        properties = {
            ...
            self.date_field: self.service.build_date_property(periodo["start"], periodo.get("end")),
            "Data de Lançamento": self.service.build_date_property(data_lancamento),
        }

        if resumo_episodio and self.description_field:
            properties[self.description_field] = self.service.build_rich_text_property(
                resumo_episodio
            )
    ...
    async def reschedule_episode(
        ...
        if new_recording_date is None and new_publication_date is None:
            raise ValueError("Provide at least one new recording or publication date")
        ...
        if not properties:
            raise ValueError("No updates were provided for the episode")
    ...
    async def schedule_recordings(
        ...
        titles_base = base_title or "Episode"
    ...
    def _parse_datetime(value: Any) -> datetime:
        ...
        if isinstance(value, str):
            try:
                ...
                raise ValueError(
                    "Recording and publication dates must follow ISO 8601 format (YYYY-MM-DDTHH:MM)."
                ) from exc

        raise TypeError("Recording/publication dates must be datetime objects or ISO 8601 strings.")
    ...
    def _normalize_recording_start(self, recording_start: datetime) -> datetime:
        ...
        if normalized.hour != expected_hour or normalized.minute != expected_minute:
            raise ValueError(
                "Recordings must start at 21:00 to respect the channel routine."
            )
    ...
    def _validate_publication_after_recording(recording_end: datetime, publication_date: datetime) -> None:
        if publication_date <= recording_end:
            raise ValueError(
                "Publication date must be later than the recording end (after 23:50)."
            )
