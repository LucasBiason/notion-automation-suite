...
from notion_mcp.utils import WorkStatus, Priority
...
async def test_create_project_with_defaults(work_notion):
    ...
    with patch.object(work_notion.service, 'create_page', new=AsyncMock(return_value=mock_page)):
        result = await work_notion.create_card(
            title="Test Project"
        )
    
    # Verify defaults
    call_args = work_notion.service.create_page.call_args
...
async def test_create_subitem_with_relation(work_notion):
    ...
    with patch.object(work_notion.service, 'create_page', new=AsyncMock(return_value=mock_page)):
        result = await work_notion.create_subitem(
...
async def test_update_status(work_notion):
    ...
    with patch.object(work_notion.service, 'update_page', new=AsyncMock(return_value=mock_page)):
        result = await work_notion.update_status(
            page_id="test_id",
            status="Concluído"
        )
    
    call_args = work_notion.service.update_page.call_args
    properties = call_args.kwargs['properties']
